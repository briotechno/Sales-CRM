...
const AnnouncementCategory = {
    create: async (data, userId) => {
        ...
        const [existing] = await pool.query('SELECT id FROM announcement_categories WHERE name = ? AND user_id = ?', [name, userId]);
        if (existing.length > 0) {
            throw new Error('Category with this name already exists');
        }

        const [result] = await pool.query(
            'INSERT INTO announcement_categories (name, status, user_id) VALUES (?, ?, ?)',
    ...
    findAll: async (userId, page = 1, limit = 10, status = null, search = '') => {
        ...
        let query = 'SELECT * FROM announcement_categories WHERE user_id = ?';
        let countQuery = 'SELECT COUNT(*) as total FROM announcement_categories WHERE user_id = ?';
    ...
    findById: async (id, userId) => {
        const [rows] = await pool.query('SELECT * FROM announcement_categories WHERE id = ? AND user_id = ?', [id, userId]);
        return rows[0];
    },
    ...
    update: async (id, data, userId) => {
        const { name, status } = data;
        const [result] = await pool.query(
            'UPDATE announcement_categories SET name = ?, status = ? WHERE id = ? AND user_id = ?',
