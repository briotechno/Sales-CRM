    ...
    useEffect(() => {
        if (plan) {
            setForm({
                ...
                key_features: plan.key_features ? (typeof plan.key_features === 'string' ? plan.key_features.split(',').map(f => f.trim()).filter(f => f) : plan.key_features) : [],
                price: plan.price || 0,
                default_users: plan.default_users || 0,
                monthly_leads: plan.monthly_leads || 0,
                default_storage: plan.default_storage || 0,
                upgradable_users: plan.upgradable_users || false,
                upgradable_storage: plan.upgradable_storage || false,
                upgradable_leads: plan.upgradable_leads || false,
                price_per_user: plan.price_per_user || 0,
                price_per_gb: plan.price_per_gb || 0,
                price_per_lead: plan.price_per_lead || 0,
                discount_12_months: plan.discount_12_months || 0,
                discount_6_months: plan.discount_6_months || 0,
                discount_3_months: plan.discount_3_months || 0,
            });
        }
    }, [plan]);
