...
const EditTeamModal = ({ isOpen, onClose, onSubmit, isLoading: isUpdating, teamId }) => {
    const [formData, setFormData] = useState({
        teamName: "",
        description: "",
        selectedEmployees: [], // for compatibility
        status: "Active",
    ...
    useEffect(() => {
        if (team) {
            const initialLevels = team.members && team.members.length > 0
                ? team.members.map(m => ({
                    id: m.id,
                ...
                : [{ id: Date.now(), departmentId: "All", designationId: "All", employeeId: "" }];
                ...
                {isFetchingTeam ? (
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-5 text-left">
                        ...
                        <div className="flex gap-3 pt-4 border-t-2 border-gray-100">
                            <button
                                type="submit"
                                disabled={isUpdating || !formData.levels.some(l => l.employeeId)}
